...
module.exports = {
  ...
  async post(req, res) {
    try {
      const { name, email, admin,cpf_cnpj,telefone,vips } = req.body;
      ...
      await mailer.sendMail({
        to: user.email,
        from: "no-reply@divinareceita.com.br",
        subject: "Divina Receita - Senha provisória",
        html: `<h2>Olá ${user.name},</h2>

        <p>
        Você recebeu um convite para participar do melhor site de receitas do Brasil, o Divina Receita.
  ...
  async put(req, res) {
    try {
      let { id, name, email, admin = false, vips=false  } = req.body;

      await User.update(id, { name, email, is_admin: admin, is_vips: vips });
